...
ProgramName = 'File Updater GUI v1.4.3'
...
windowlayout = [
    ...
    [gui.T(text='IP', size=(4)), gui.I(key='IP', default_text='localhost', size=(15), tooltip='Host\'s IP'), gui.T(size=18), gui.Combo(key='CompressionType', values=['ZIP', 'BZIP2', 'LZMA'], tooltip='Compression Type', default_value='ZIP', size=5)],
    [gui.T(text='PORT', size=(4)), gui.I(key='PORT', default_text='6969', size=(5), tooltip='Host\'s PORT'), gui.T(size=18), gui.Checkbox(key='CompressionOpt', default=False, text='File Compression', tooltip='Requires more storage during hosting/downloading (Must be enabled on server and client to work)')],
