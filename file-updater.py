...
ProgramName = 'File Updater GUI v1.4.4'
...
CompressionLayout = [
    ...
    [gui.Checkbox(key='CompressionOpt', default=False, text='File Compression', tooltip='Requires more storage during hosting/downloading (Must be enabled on server and client to work)')]
]
LargeFileLayout = [
    [gui.Combo(key='LargeFileA/M', values=('Auto', 'Manual'), size=6, default_value='Auto', tooltip='Large File Thershold', readonly=True, enable_events=True)],
...
def checksumfiles(directory, largefilelimit, largefileopt):
    ...
    for file in filedirlist:
        with open(file=file, encoding='iso-8859-1') as filecheck:
            ...
            if largefileopt == 'Manual' and largefilelimit <= os.path.getsize(file)//1024000:
